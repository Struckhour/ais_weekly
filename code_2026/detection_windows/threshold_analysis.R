...
avg <- mean(wilcox_results_df$prob_superiority)


threshold_summary_df <- purrr::map_dfr(
  ...
  function(thresh) {
    ...
    tibble::tibble(
      threshold = thresh,
      mean_prob_superiority = mean(wilcox_results_df$prob_superiority, na.rm = TRUE),
      n_tests = sum(!is.na(wilcox_results_df$prob_superiority))
    )
  }
)

ggplot(threshold_summary_df, aes(x = threshold, y = mean_prob_superiority)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_x_continuous(
    breaks = seq(0.70, 0.99, by = 0.01),
    labels = scales::percent_format(accuracy = 1)
  ) +
  theme_classic() +
  labs(
    title = "Average probability of superiority across thresholds",
    x = "Window threshold",
    y = "Mean probability of superiority"
  )
...
threshold_summary_df <- purrr::map_dfr(
  ...
  function(thresh) {
    ...
    vals <- wilcox_results_df$prob_superiority

    tibble::tibble(
      threshold = thresh,
      mean_prob_superiority = mean(vals, na.rm = TRUE),
      min_prob_superiority  = min(vals, na.rm = TRUE),
      max_prob_superiority  = max(vals, na.rm = TRUE),
      n_tests = sum(!is.na(vals))
    )
  }
)


ggplot(threshold_summary_df, aes(x = threshold)) +
  geom_line(aes(y = mean_prob_superiority), linewidth = 1) +

  geom_point(aes(y = mean_prob_superiority), size = 2) +
  geom_point(aes(y = min_prob_superiority), size = 2, alpha = 0.6) +
  geom_point(aes(y = max_prob_superiority), size = 2, alpha = 0.6) +
  ...
  theme_classic() +
  labs(
    title = "Probability of superiority across thresholds",
    x = "Window threshold",
...
ggplot(threshold_summary_df, aes(x = threshold)) +
  geom_linerange(
    aes(ymin = min_prob_superiority, ymax = max_prob_superiority),
    linewidth = 0.8,
    alpha = 0.6
  ) +
  geom_line(aes(y = mean_prob_superiority), linewidth = 1) +
...
threshold_values_df <- purrr::map_dfr(
  ...
  function(thresh) {
    ...
    # ---- DEBUG: print low values ----
    low_vals <- wilcox_results_df %>%
      dplyr::filter(prob_superiority < 0.6)

    if (nrow(low_vals) > 0) {
      purrr::pwalk(
        low_vals,
        function(species, region, prob_superiority, ...) {
          message(
            sprintf(
              "Threshold %.2f | %s - %s | prob_sup = %.3f",
              thresh, region, species, prob_superiority
            )
          )
        }
      )
...
ggplot(
  ...
  aes(
    ...
    y = prob_superiority,
    color = region,
    group = interaction(region, species)
  )
) +
  geom_line(alpha = 0.4, linewidth = 0.6) +
  geom_jitter(width = 0.002, height = 0, alpha = 0.6, size = 1.5) +
...
ggplot(
  ...
  geom_line(alpha = 0.4, linewidth = 0.6) +
  geom_jitter(width = 0.002, height = 0, alpha = 0.7, size = 2) +
...
ggplot(threshold_avg_from_values_df,
       aes(x = threshold, y = mean_prob_superiority)) +
  geom_ribbon(
...
ggplot(threshold_avg_from_values_df,
  ...
  scale_y_continuous(
    name = "Mean probability of superiority",
    sec.axis = sec_axis(~ ., name = "Proportion valid Wilcoxon tests")
  ) +
  theme_classic() +
  labs(
    title = "Probability of superiority and valid test coverage across thresholds",
    x = "Window threshold"
  )
...
threshold_pval_df <- threshold_values_df %>%
  dplyr::group_by(threshold) %>%
  dplyr::summarise(
...
ggplot(threshold_pval_df, aes(x = threshold, y = median_p_value)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_hline(
    yintercept = 0.05,
  ...
  theme_classic() +
  labs(
    title = "Median Wilcoxon p-value across thresholds",
    x = "Window threshold",
    y = "Median p-value"
  )

ggplot(threshold_pval_df, aes(x = threshold, y = prop_significant)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1),
  ...
  theme_classic() +
  labs(
    title = "Proportion of significant Wilcoxon tests across thresholds",
    x = "Window threshold",
    y = "Proportion p < 0.05"
  )
